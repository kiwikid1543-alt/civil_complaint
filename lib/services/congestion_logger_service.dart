...
class CongestionLoggerService {
  ...
  Future<void> _fetchAndLog() async {
    ...
    // 대상 CSO Sn: CS0005(서초구청), CS0041(부산남구), CS0001(광진구청)
    final targets = ['CS0005', 'CS0041', 'CS0001'];
    ...
    for (var csoSn in targets) {
      try {
        ...
        result.fold(
          (status) async {
            // Task 리스트 추출
            final tasks = status.taskStatuses
                .map(
                  (t) => {'taskNm': t.taskNm, 'waitingCount': t.waitingCount},
                )
                .toList();
            ...
            final timeSlot =
                '${now.hour.toString().padLeft(2, '0')}:$minuteSlot';
